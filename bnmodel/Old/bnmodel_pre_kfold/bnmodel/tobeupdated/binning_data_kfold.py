...
def binning_data(df, n_folds):
    ...
    for col in x_df.columns:
        col_bins = col + '_bins'
        # Define the bin edges using the specified strategy
        bin_edges = pd.cut(x_df.loc[:, col], bins=number_of_bins, labels=False, retbins=True, duplicates='drop')[1]
        # Apply binning to the column using the defined bin edges
        x_df.loc[:, col_bins] = pd.cut(x_df.loc[:, col], bins=bin_edges, labels=labels, include_lowest=True)
    ...
    for col in y_df.columns:
        col_bins = col + '_bins'
        # Define the bin edges using the specified strategy
        bin_edges = pd.qcut(y_df.loc[:, col], q=number_of_bins, labels=False, retbins=True, duplicates='drop')[1]
        # Apply binning to the column using the defined bin edges
        y_df.loc[:, col_bins] = pd.cut(y_df.loc[:, col], bins=bin_edges, labels=labels, include_lowest=True)
        bin_edges_dict[col_bins] = bin_edges
...
dfs_binned, dfs_test_xy, dfs_test_x, bin_edges_dict, prior_dict_xytrn = binning_data(df, 5)
print(dfs_binned)