...
- use of dictionaries for training testing data so that model does not run every time script runs
...
# Create k-fold cross-validator object
kf = KFold(n_splits=n_folds, shuffle=True, random_state=42)

# Split data into folds
fold_indices = list(kf.split(x_df))

# Define kfold cross-validator with 5 splits
...
for fold in range(n_folds):
    ...
    x_train, x_test = x_df.iloc[train_indices], x_df.iloc[test_indices]
    y_train, y_test = y_df.iloc[train_indices], y_df.iloc[test_indices]
    
    # Train and test the model using the current fold's data
    # Insert your model training and testing code here
    
    # Evaluate the model's performance on the current fold's test set
